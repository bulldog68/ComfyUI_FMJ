...
folder_paths = {
    "csv": os.path.join(script_directory, "csv")
}

def new_random_seed(seed):
    """Generate a new random seed using the provided seed for reproducibility."""
    rng = random.Random(seed)
    return rng.randint(1, 1125899906842624)

def getfilename(folder):
    """Get list of CSV filenames (without prefix/suffix) in the specified folder."""
    try:
        return [filename[3:-4] for filename in sorted(os.listdir(folder)) if filename.endswith(".csv")]
    except Exception as e:
        print(f"➡️CreaPrompt: Erreur lors de la lecture du dossier {folder}: {e}")
        return []

def select_random_line_from_collection(seed):
    """Select a random line from collection.txt using the provided seed."""
    rng = random.Random(seed)
    ...
    try:
        ...
        print(f"➡️CreaPrompt: Erreur lors de la lecture de collection.txt: {e}")
        return ""

def select_random_line_from_csv_file(category, folder, seed):
    """Select a random line from a CSV file using the provided seed."""
    rng = random.Random(seed)
    try:
        ...
        print(f"➡️CreaPrompt: Erreur lors de la lecture de {category}: {e}")
        return ""
...
class CreaPrompt:
    ...
    def INPUT_TYPES(cls):
        ...
        try:
            ...
            print(f"➡️CreaPrompt: Erreur lors de la lecture du dossier csv: {e}")
    ...
    @classmethod
    def IS_CHANGED(cls, seed, CreaPrompt_Collection, prefix, suffix, **kwargs):
        """Return a hash of inputs to prevent unnecessary regeneration."""
        input_str = f"{seed}:{CreaPrompt_Collection}:{prefix}:{suffix}:{json.dumps(kwargs, sort_keys=True)}"
        return hashlib.sha256(input_str.encode()).hexdigest()

    def create_prompt(self, seed=0, CreaPrompt_Collection="disabled", prefix="", suffix="",
                     prompt=None, extra_pnginfo=None, unique_id=None, node=None, **kwargs):
        """Generate prompts with deterministic seed handling."""
        ...
        # Handle special seed values (-1, -2, -3) for randomization
        original_seed = seed
        if seed in (-1, -2, -3):
            ...
            if unique_id and extra_pnginfo and isinstance(extra_pnginfo, list) and len(extra_pnginfo) > 0:
                try:
                    ...
                    print(f"➡️CreaPrompt: Erreur lors de la mise à jour des métadonnées du workflow: {e}")
            ...
            if prompt and str(unique_id) in prompt and 'inputs' in prompt[str(unique_id)] and 'seed' in prompt[str(unique_id)]['inputs']:
                try:
                    ...
                    print(f"➡️CreaPrompt: Erreur lors de la mise à jour du prompt: {e}")
        ...
        print(f"➡️CreaPrompt: Prompt généré (exécution {execution_id}): {final_values_str}")
        print(f"➡️CreaPrompt: Graine finale: {seed}")
