...
folder_paths = {
    "csv": os.path.join(script_directory, "csvkontext")
}
...
def getfilename(folder):
    try:
        ...
        print(f"➡️FMJ Kontext: Erreur lors de la lecture du dossier {folder}: {e}")
        return []
...
def select_random_line_from_collection(seed):
    ...
    try:
        ...
        print(f"➡️FMJ Kontext: Erreur lors de la lecture de collection.txt: {e}")
        return ""
...
def select_random_line_from_csv_file(category, folder, seed):
    ...
    try:
        ...
        print(f"➡️FMJ Kontext: Erreur lors de la lecture de {category}: {e}")
        return ""
...
class FMJKontext:
    ...
    def INPUT_TYPES(cls):
        ...
        try:
            ...
            print(f"➡️FMJ Kontext: Erreur lors de la lecture du dossier csvkontext: {e}")

        return {
            "required": required,
            "optional": {
                "Kontext_Prompt_Collection": (["disabled", "enabled"], {"default": "disabled"}),
    ...
    @classmethod
    def IS_CHANGED(cls, seed, Kontext_Prompt_Collection, prefix, suffix, **kwargs):
        input_str = f"{seed}:{Kontext_Prompt_Collection}:{prefix}:{suffix}:{json.dumps(kwargs, sort_keys=True)}"
        return hashlib.sha256(input_str.encode()).hexdigest()

    def create_prompt(self, seed=0, Kontext_Prompt_Collection="disabled", prefix="", suffix="",
        ...
        if seed in (-1, -2, -3):
            ...
            if unique_id and extra_pnginfo and isinstance(extra_pnginfo, list) and len(extra_pnginfo) > 0:
                try:
                    ...
                    print(f"➡️FMJ Kontext: Erreur lors de la mise à jour des métadonnées du workflow: {e}")
            ...
            if prompt and str(unique_id) in prompt and 'inputs' in prompt[str(unique_id)] and 'seed' in prompt[str(unique_id)]['inputs']:
                try:
                    ...
                    print(f"➡️FMJ Kontext: Erreur lors de la mise à jour du prompt: {e}")

        if Kontext_Prompt_Collection == "enabled":
        ...
        print(f"➡️FMJ Kontext: Prompt généré (exécution {execution_id}): {final_values_str}")
        print(f"➡️FMJ Kontext: Graine finale: {seed}")
